...
get_default_ss_config() {
    local _ss
    _ss=$(ls "$is_conf_dir" 2>/dev/null | grep -E '^Shadowsocks-.*\.json$' | head -n1)
    [[ ! $_ss ]] && _ss=$(ls "$is_conf_dir" 2>/dev/null | grep -E '\.json$' | grep -v '^config\.json$' | grep -v 'dynamic-port-.*-link' | head -n1)
    [[ $_ss ]] && echo "$_ss"
}

show_ss_info() {
    local _cfg=${1:-$(get_default_ss_config)}
    [[ ! $_cfg ]] && err "无法找到 Shadowsocks 配置."
    info "$_cfg"
}
...
is_main_menu() {
    msg "\n------------- $is_core_name SS-only $is_sh_ver by $author -------------"
    msg "$is_core_ver: $is_core_status"
    is_main_start=1
    ask mainmenu
    case $REPLY in
    1)
        add ss
        [[ $is_config_name ]] && url_qr qr "$is_config_name"
        ;;
    2)
        local _cfg=$(get_default_ss_config)
        [[ ! $_cfg ]] && err "无法找到 Shadowsocks 配置."
        change "$_cfg"
        ;;
    3)
    ...
    5)
        ...
        ;;
    6)
        ask list is_do_manage "启动 停止 重启 查看状态 查看日志 查看错误日志"
        case $REPLY in
        1) manage start & msg "\n管理状态执行: $(_green 启动)\n" ;;
        2) manage stop & msg "\n管理状态执行: $(_green 停止)\n" ;;
        3) manage restart & msg "\n管理状态执行: $(_green 重启)\n" ;;
        4) msg "\n$is_core_ver: $is_core_status\n" ;;
        5) load log.sh; log_set log ;;
        6) load log.sh; log_set logerr ;;
        esac
        ;;
    7)
        update core
...
main() {
    ...
    a | add)
        ...
        [[ ! $is_gen && ! $is_no_auto_tls && $is_config_name ]] && url_qr qr "$is_config_name"
        ;;
    gen | no-auto-tls)
        [[ $1 == 'gen' ]] && is_gen=1
        [[ $1 == 'no-auto-tls' ]] && is_no_auto_tls=1
    ...
    un | uninstall)
        ...
        ;;
    u | up | update)
        if [[ ! $2 || $2 == core || $2 == $is_core || $2 == 1 ]]; then
            update core $3
        elif [[ $2 == dat ]]; then
            load download.sh
            download dat
            ...
            err "精简版仅支持: $is_core update [core | dat]"
        fi
        ;;
    s | status)
        msg "\n$is_core_ver: $is_core_status\n"
        ;;
    start | stop | r | restart)
        [[ $2 && $2 != 'caddy' ]] && err "无法识别 ($2), 请使用: $is_core $1"
        [[ $1 == "r" ]] && set -- restart
        manage $1 &
        ;;
    t | test)
    ...
    *)
        ...
        if [[ $is_change_id ]]; then
            unset is_try_change
            [[ $2 ]] && change $2 $1 ${@:3} || change "$(get_default_ss_config)" $1 ${@:2}
        else
            err "无法识别 ($1), 获取帮助请使用: $is_core help"
        fi
        ;;

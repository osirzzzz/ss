...
# github=https://github.com/osirzzzz/ss
...
is_sh_repo=$repo_owner/$repo_name
...
download() {
    ...
    sh)
        ...
        for f in "${files[@]}"; do
            mkdir -p "$(dirname "$tmpshdir/$f")"
            link="${raw_base}/${f}"
            msg warn "下载脚本 > ${link}"
            _wget -t 3 -q -c "$link" -O "$tmpshdir/$f" || return 1
        done
        >"$is_sh_ok"
        ;;
    esac
...
check_status() {
    ...
    if [[ $is_wget ]]; then
        ...
        [[ $jq_not_found && ! -f $is_jq_ok ]] && { msg err "下载 jq 失败"; is_fail=1; }
    else
        [[ ! $is_fail ]] && {
            is_wget=1
...
pass_args() {
    while [[ $# -gt 0 ]]; do
        case $1 in
        -f | --core-file)
            [[ -z $2 ]] && err "($1) 缺少必需参数, 正确使用示例: [$1 /root/$is_core-linux-64.zip]"
            [[ ! -f $2 ]] && err "($2) 不是一个常规的文件."
            is_core_file=$2
            shift 2
            ;;
        -l | --local-install)
            [[ ! -f ${PWD}/src/core.sh || ! -f ${PWD}/src/init.sh || ! -f ${PWD}/src/systemd.sh || ! -f ${PWD}/$is_core.sh ]] && {
            ...
            ;;
        -p | --proxy)
            [[ -z $2 ]] && err "($1) 缺少必需参数, 正确使用示例: [$1 http://127.0.0.1:2333]"
            proxy=$2
            shift 2
            ;;
        -v | --core-version)
            [[ -z $2 ]] && err "($1) 缺少必需参数, 正确使用示例: [$1 v5.4.1]"
            is_core_ver=v${2#v}
            shift 2
            ;;
        -h | --help)
...
main() {
    ...
    grep -q "alias $is_core=$is_sh_bin" /root/.bashrc 2>/dev/null || echo "alias $is_core=$is_sh_bin" >>/root/.bashrc
    ...
    find "$is_sh_dir" -type f -name '*.sh' -exec chmod +x {} \;
    chmod +x "$is_core_bin" "$is_sh_bin" 2>/dev/null || true
